...
export default function AdminPage() {
  ...
  useEffect(() => {
    if (selectedMovie) {
      setFormData({
        ...
        releaseDate: selectedMovie.releaseDate ? selectedMovie.releaseDate.split('T')[0] : new Date().toISOString().split('T')[0],
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
    ...
    setFormData(prev => ({
      ...prev,
      [name]: name === 'rating' || name === 'duration' ? parseInt(value, 10) || 0 : value,
    }));
  };
  ...
  const closeModals = () => {
    ...
    // Use a timeout to avoid UI flashing when switching between modals
    setTimeout(() => {
      setSelectedMovie(null);
    }, 200);
  };
  ...
  if (loading && movies.length === 0) {
    return <LoadingSpinner fullScreen size="lg" message="Loading movies..." />;
  }

  if (error && movies.length === 0) {
    return <ErrorDisplay message={error} onRetry={loadMovies} />;
  }
